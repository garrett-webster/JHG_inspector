...
class TestGameFileLoader_JsonV1:
    ...
    def test_load_searchTags_data(self, game, cursor):
        ...
        cursor.execute("SELECT tag FROM searchTags")
        actual_search_tags = cursor.fetchall()
        ...
        cursor.execute("SELECT value FROM searchTags")
        actual_search_tag_values = cursor.fetchall()
    ...
    def test_load_data_to_database_influences(self, game, cursor):
        ...
        cursor.execute("SELECT * FROM influences")
        actual_influences = cursor.fetchall()
    ...
    def test_load_chatParticipants_data(self, game, cursor):
        ...
        for i, chat_id in enumerate(chat_ids):
            cursor.execute("SELECT playerId FROM chatParticipants WHERE conversationId = ?", chat_id)
            actual_participants = cursor.fetchall()
